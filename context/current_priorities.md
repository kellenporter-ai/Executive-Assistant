...
- **Status (2026-03-14):** Two major frontend rounds complete. Round 1: toast notifications, command palette, keyboard shortcuts, welcome/onboarding, breadcrumbs, minimap, spellcheck. Round 2 (VS Code-inspired): status bar, enhanced notifications (severity-based, hover-pause, notification center), tab enhancements (context menu, pin/sticky, dirty indicator, drag-to-reorder), accessibility overhaul (dual ARIA live regions, focus trap, aria-activedescendant, prefers-reduced-motion), enhanced command palette (MRU history, grouped results, fuzzy highlighting), sidebar resize. Backend: fixed session archive/delete persistence (`_delete_from_cli()` helper). Single-file frontend is ~4900 lines. QA passed all rounds.
